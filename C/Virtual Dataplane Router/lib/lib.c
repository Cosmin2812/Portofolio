...
struct route_table_entry *get_best_route(uint32_t ip_dest, struct route_table_entry *rtable, int rtable_len) {
	qsort((void *)rtable, rtable_len, sizeof(struct route_table_entry), prefix_comparator); 
...
struct arp_table_entry *get_mac_entry(uint32_t given_ip, struct arp_table_entry *arp_table, int arp_table_len) {
...
void convert_to_int(char* initial_ip, uint32_t *ip_address) {
	char* token;
	//char s[2] = ".";
	token = strtok(initial_ip, ".");
	int i = 0;
	while(token != NULL) {
		int x = atoi(token);
		*((uint8_t *) ip_address + i) = x;
		//printf("%s", token);
		token = strtok(NULL, ".");
		i++;
	}
